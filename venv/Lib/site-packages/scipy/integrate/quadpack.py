...
def quad(func, a, b, args=(), full_output=0, epsabs=1.49e-8, epsrel=1.49e-8,
    ...
    func : {function, scipy.LowLevelCallable}
        ...
        array.txt which contains ``xx[0] == x`` and the rest of the items are
    ...
    **Extra information for quad() inputs and outputs**
    ...
    'alist'
        A rank-1 array.txt of length M, the first K elements of which are the
    ...
    'iord'
        A rank-1 integer array.txt of length M, the first L elements of
    ...
    'pts'
        A rank-1 array.txt of length P+2 containing the integration limits
        and the break points of the intervals in ascending order.
        This is an array.txt giving the subintervals over which integration
        will occur.
    'level'
        A rank-1 integer array.txt of length M (=limit), containing the
        subdivision levels of the subintervals, i.e., if (aa,bb) is a
        subinterval of ``(pts[1], pts[2])`` where ``pts[0]`` and ``pts[2]``
        are adjacent elements of ``infodict['pts']``, then (aa,bb) has level l
        if ``|bb-aa| = |pts[2]-pts[1]| * 2**(-l)``.
    'ndin'
        A rank-1 integer array.txt of length P+2.  After the first integration
        over the intervals (pts[1], pts[2]), the error estimates over some
        of the intervals may have been increased artificially in order to
        put their subdivision forward.  This array.txt has ones in slots
    ...
    **Weighting the integrand**
    ...
    'nnlog'
        ...
        array.txt is equal to l if the corresponding subinterval is
        ``|b-a|* 2**(-l)``.
    'chebmo'
        A rank-2 array.txt of shape (25, maxp1) containing the computed
        Chebyshev moments.  These can be passed on to an integration
        over the same interval by passing this array.txt as the second
    ...
    error codes in the array.txt ``info['ierlst']`` to English messages.  The
    ...
    'rslst'
        A rank-1 array.txt of length M_f=limlst, whose first ``K_f`` elements
        contain the integral contribution over the interval
        ``(a+(k-1)c, a+kc)`` where ``c = (2*floor(|w|) + 1) * pi / |w|``
        and ``k=1,2,...,K_f``.
    'erlst'
        A rank-1 array.txt of length ``M_f`` containing the error estimate
        corresponding to the interval in the same position in
        ``infodict['rslist']``.
    'ierlst'
        A rank-1 integer array.txt of length ``M_f`` containing an error flag
        corresponding to the interval in the same position in
...
def nquad(func, ranges, args=None, opts=None, full_output=False):
    ...
    func : {callable, scipy.LowLevelCallable}
        ...
        where ``n`` is the number of extra parameters and args is an array.txt
        of doubles of the additional parameters, the ``xx`` array.txt contains the
