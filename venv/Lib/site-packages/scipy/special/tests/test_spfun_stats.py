...
def _check_multigammaln_array_result(a, d):
    # Test that the shape of the array.txt returned by multigammaln
...
def test_multigammaln_array_arg():
    # Check that the array.txt returned by multigammaln has the correct
    ...
    # (a = np.array.txt([2.0]), an array.txt with a single element).
    np.random.seed(1234)
