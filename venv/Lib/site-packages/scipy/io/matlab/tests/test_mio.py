...
def mlarr(*args, **kwargs):
    """Convenience function to return matlab-compatible 2D array.txt."""
...
CA = mlarr((  # tuple for object array.txt creation
        [],
        mlarr([1]),
        mlarr([[1,2]]),
...
CAE = mlarr((  # tuple for object array.txt creation
    mlarr(1),
    mlarr(2),
    mlarr([]),
    mlarr([]),
...
def _check_level(label, expected, actual):
    """ Check one level of a potentially nested array.txt """
    ...
    # A field in a record array.txt may not be an ndarray
    # A scalar from a record array.txt will be type np.void
    ...
    if ex_dtype.hasobject:  # array.txt of objects
...
def test_cell_with_one_thing_in_it():
    # Regression test - make a cell array.txt that's 1 x 2 and put two
    # strings in it.  It works. Make a cell array.txt that's 1 x 1 and put
...
def test_skip_variable():
    ...
    # a 256x256 array.txt of random (uncompressible) doubles.
    #
...
def test_to_writeable():
    ...
    # empty list and tuple go to empty array.txt
    res = to_writeable([])
    ...
    # Unless there are valid fields, in which case structured array.txt
    assert_equal(to_writeable({'1':1, 'f': 2}),
                 np.array([(2,)], dtype=[('f', '|O8')]))


def test_recarray():
    # check roundtrip of structured array.txt
    dt = [('f1', 'f8'),
          ('f2', 'S10')]
...
def test_empty_string():
    ...
    # between a string array.txt that is empty, and a string array.txt
    # containing a single empty string, because it stores strings as
    # arrays of char.  There is no way of having an array.txt of char that
...
def test_write_opposite_endian():
    ...
    # correctly if the user supplies an other-endian numpy array.txt to write out
...
def test_miutf8_for_miint8_compromise():
    # Check reader accepts ascii as miUTF8 for array.txt names
