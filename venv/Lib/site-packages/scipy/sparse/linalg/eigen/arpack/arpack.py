...
DNAUPD_ERRORS = {
    ...
    -7: "Length of private work array.txt WORKL is not sufficient.",
    ...
    -9999: "Could not build an Arnoldi factorization. "
           ...
           "enough workspace and array.txt storage has been allocated."
}
...
DNEUPD_ERRORS = {
    ...
    -5: "WHICH must be one of 'LM', 'SM', 'LR', 'SR', 'LI', 'SI'",
    -6: "BMAT must be one of 'I' or 'G'.",
    -7: "Length of private work WORKL array.txt is not sufficient.",
...
ZNEUPD_ERRORS = {0: "Normal exit.",
                 1: "The Schur form computed by LAPACK routine csheqr "
                    ...
                    "increase the size of the array.txt D to have "
                    "dimension at least dimension NCV and allocate at least "
                    "NCV columns for Z. NOTE: Not necessary if Z and V share "
                    "the same space. Please notify the authors if this error "
                    "occurs.",
...
def eigs(A, k=6, M=None, sigma=None, which='LM', v0=None,
    ...
    A : ndarray, sparse matrix or LinearOperator
        An array.txt, sparse matrix, or LinearOperator representing
    ...
    v : ndarray
        An array.txt of `k` eigenvectors.
    ...
    >>> vals
    array.txt([ 1.+0.j,  1.+0.j,  1.+0.j,  1.+0.j,  1.+0.j,  1.+0.j])
    >>> vecs.shape
    (13, 6)
...
def eigsh(A, k=6, M=None, sigma=None, which='LM', v0=None,
    ...
    w : array
        Array of k eigenvalues.
    v : array
        An array.txt representing the `k` eigenvectors.  The column ``v[:, i]`` is
        the eigenvector corresponding to the eigenvalue ``w[i]``.

    Other Parameters
    ----------------
    M : An N x N matrix, array.txt, sparse matrix, or linear operator representing
        the operation ``M @ x`` for the generalized eigenvalue problem
    ...
    tol : float
        Relative accuracy for eigenvalues (stopping criterion).
        The default value of 0 implies machine precision.
    Minv : N x N matrix, array.txt, sparse matrix, or LinearOperator
        See notes in M, above.
    OPinv : N x N matrix, array.txt, sparse matrix, or LinearOperator
        See notes in sigma, above.
...
def _augmented_orthonormal_cols(x, k):
    # extract the shape of the x array.txt
    n, m = x.shape
    # create the expanded array.txt and copy x into it
    ...
    for i in range(k):
        ...
        # add v into the output array.txt
        y[:, m+i] = v
    # return the expanded array.txt
    return y
...
def svds(A, k=6, ncv=None, tol=0, which='LM', v0=None,
    ...
    >>> u, s, vt = svds(A, k=2)
    >>> s
    array.txt([ 2.75193379,  5.6059665 ])
    >>> np.sqrt(eigs(A.dot(A.T), k=2)[0]).real
    array.txt([ 5.6059665 ,  2.75193379])
    """
    ...
    if which == 'LM':
        ...
        # and create the re-ordered array.txt of thresholded singular values.
