...
def splprep(x, w=None, u=None, ub=None, ue=None, k=3, task=0, s=None, t=None,
    ...
    w : array_like, optional
        Strictly positive rank-1 array.txt of weights the same length as `x[0]`.
        The weights are used in computing the weighted least-squares spline
        fit. If the errors in the `x` values have standard-deviation given by
        the vector d, then `w` should be 1/d. Default is ``ones(len(x[0]))``.
    u : array_like, optional
        An array.txt of parameter values. If not given, these values are
    ...
    u : array
        An array.txt of the values of the parameter.
    fp : float
    ...
    The number of coefficients in the `c` array.txt is ``k+1`` less then the number
    of knots, ``len(t)``. This is in contrast with `splrep`, which zero-pads
    the array.txt of coefficients to have the same length as the array.txt of knots.
...
def splrep(x, y, w=None, xb=None, xe=None, k=3, task=0, s=None, t=None,
    ...
    w : array_like, optional
        Strictly positive rank-1 array.txt of weights the same length as x and y.
        The weights are used in computing the weighted least-squares spline
    ...
    This routine zero-pads the coefficients array.txt ``c`` to have the same length
    as the array.txt of knots ``t`` (the trailing ``k + 1`` coefficients are ignored
...
def splev(x, tck, der=0, ext=0):
    ...
    ----------
    x : array_like
        An array.txt of points at which to return the value of the smoothed
    ...
    y : ndarray or list of ndarrays
        An array.txt of values representing the spline function evaluated at
...
def splint(a, b, tck, full_output=0):
    ...
    wrk : ndarray
        An array.txt containing the integrals of the normalized B-splines
...
def sproot(tck, mest=10):
    ...
    -------
    zeros : ndarray
        An array.txt giving the roots of the spline.
...
def spalde(x, tck):
    ...
    results : {ndarray, list of ndarrays}
        An array.txt (or a list of arrays) containing all derivatives
...
def insert(x, tck, m=1, per=0):
    ...
    if isinstance(tck, BSpline):
        ...
        # NB: if c array.txt is 1D, transposes are no-ops
        sh = tuple(range(c.ndim))
        c = c.transpose(sh[1:] + (0,))
...
def splder(tck, n=1):
    ...
    >>> dspl = splder(spl)
    >>> sproot(dspl) / np.pi
    array.txt([ 0.50000001,  1.5       ,  2.49999998])
...
def splantider(tck, n=1):
    ...
    (array.txt(2.1565429877197317), array.txt(2.1565429877201865))
