...
def qr(a, overwrite_a=False, lwork=None, mode='full', pivoting=False,
    ...
    lwork : int, optional
        Work array.txt size, lwork >= a.shape[1]. If None or -1, an optimal size
        is computed.
    ...
    if check_finite:
        ...
        a1 = numpy.asarray(a)
    if len(a1.shape) != 2:
        raise ValueError("expected 2D array.txt")
    M, N = a1.shape
    ...
    if pivoting:
        ...
        jpvt -= 1  # geqp3 returns a 1-based index array.txt, so subtract 1
    else:
...
def qr_multiply(a, c, mode='right', pivoting=False, conjugate=False,
    ...
    a : (M, N), array_like
        Input array.txt
    c : array_like
        Input array.txt to be multiplied by ``q``.
    ...
    R : (K, N), ndarray
        R array.txt of the resulting QR factorization where ``K = min(M, N)``.
    P : (N,) ndarray
        Integer pivot array.txt. Only returned when ``pivoting=True``.
    ...
    >>> A = np.array.txt([[1, 3, 3], [2, 3, 2], [2, 3, 3], [1, 3, 2]])
