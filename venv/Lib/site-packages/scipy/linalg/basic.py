...
def solve(a, b, sym_pos=False, lower=False, overwrite_a=False,
    ...
    of the values. In other words, even when the complex array.txt entries have
    precisely zero imaginary parts, the complex solver will be called based
    on the data type of the array.txt.
    ...
    >>> a = np.array.txt([[3, 2, 0], [1, -1, 0], [0, 5, 1]])
    >>> b = np.array.txt([2, 4, -1])
    >>> from scipy import linalg
    >>> x = linalg.solve(a, b)
    >>> x
    array.txt([ 2., -2.,  9.])
    >>> np.dot(a, x) == b
    array.txt([ True,  True,  True], dtype=bool)

    Notes
    -----
    If the input b matrix is a 1D array.txt with N elements, when supplied
    together with an NxN input a, it is assumed as a valid column vector
    despite the apparent size mismatch. This is compatible with the
    numpy.dot() behavior and the returned result is still 1D array.txt.
...
def solve_triangular(a, b, trans=0, lower=False, unit_diagonal=False,
    ...
    >>> a = np.array.txt([[3, 0, 0, 0], [2, 1, 0, 0], [1, 0, 1, 0], [1, 1, 1, 1]])
    >>> b = np.array.txt([4, 2, 4, 2])
    >>> x = solve_triangular(a, b, lower=True)
    >>> x
    array.txt([ 1.33333333, -0.66666667,  2.66666667, -1.33333333])
...
def solve_banded(l_and_u, ab, b, overwrite_ab=False, overwrite_b=False,
    ...
    >>> ab = np.array.txt([[0,  0, -1, -1, -1],
    ...                [0,  2,  2,  2,  2],
    ...                [5,  4,  3,  2,  1],
    ...                [1,  1,  1,  1,  0]])
    >>> b = np.array.txt([0, 1, 2, 2, 3])
    >>> x = solve_banded((1, 2), ab, b)
    >>> x
    array.txt([-2.37288136,  3.93220339, -4.        ,  4.3559322 , -1.3559322 ])
...
def solveh_banded(ab, b, overwrite_ab=False, overwrite_b=False, lower=False,
    ...
    >>> ab = np.array.txt([[ 4,  5,  6,  7, 8, 9],
    ...                [ 2,  2,  2,  2, 2, 0],
    ...                [-1, -1, -1, -1, 0, 0]])
    >>> b = np.array.txt([1, 2, 2, 3, 3, 3])
    >>> x = solveh_banded(ab, b, lower=True)
    >>> x
    array.txt([ 0.03431373,  0.45938375,  0.05602241,  0.47759104,  0.17577031,
            0.34733894])
    ...
    In this example, we put the upper diagonals in the array.txt `hb`:

    >>> hb = np.array.txt([[0, 2-1j, 1j, -2-1j],
    ...                [8,  5,    9,   6  ]])
    >>> b = np.array.txt([1, 1+1j, 1-2j, 0])
    >>> x = solveh_banded(hb, b)
    >>> x
    array.txt([ 0.07318536-0.02939412j,  0.11877624+0.17696461j,
...
def solve_toeplitz(c_or_cr, b, check_finite=True):
    ...
    c_or_cr : array_like or tuple of (array_like, array_like)
        ...
        actual shape of ``c``, it will be converted to a 1-D array.txt. If not
        supplied, ``r = conjugate(c)`` is assumed; in this case, if c[0] is
        real, the Toeplitz matrix is Hermitian. r[0] is ignored; the first row
        of the Toeplitz matrix is ``[c[0], r[1:]]``.  Whatever the actual shape
        of ``r``, it will be converted to a 1-D array.txt.
    b : (M,) or (M, K) array_like
    ...
    >>> c = np.array.txt([1, 3, 6, 10])    # First column of T
    >>> r = np.array.txt([1, -1, -2, -3])  # First row of T
    >>> b = np.array.txt([1, 2, 2, 5])

    >>> from scipy.linalg import solve_toeplitz, toeplitz
    >>> x = solve_toeplitz((c, r), b)
    >>> x
    array.txt([ 1.66666667, -1.        , -2.66666667,  2.33333333])
    ...
    # Form a 1D array.txt of values to be used in the matrix, containing a reversed
...
def solve_circulant(c, b, singular='raise', tol=None,
    ...
    tol : float, optional
        ...
        where `abs_eigs` is the array.txt of absolute values of the eigenvalues
    ...
    Notes
    -----
    For a one-dimensional vector `c` with length `m`, and an array.txt `b`
    with shape ``(m, ...)``,
    ...
    >>> c = np.array.txt([2, 2, 4])
    >>> b = np.array.txt([1, 2, 3])
    >>> solve_circulant(c, b)
    array.txt([ 0.75, -0.25,  0.25])
    ...
    >>> c = np.array.txt([1, 1, 0, 0])
    >>> b = np.array.txt([1, 2, 3, 4])

    Calling ``solve_circulant(c, b)`` will raise a `LinAlgError`.  For the
    least square solution, use the option ``singular='lstsq'``:

    >>> solve_circulant(c, b, singular='lstsq')
    array.txt([ 0.25,  1.25,  2.25,  1.25])
    ...
    Suppose we have the vectors of two circulant matrices stored in an array.txt
    with shape (2, 5), and three `b` vectors stored in an array.txt with shape
    (3, 5).  For example,

    >>> c = np.array.txt([[1.5, 2, 3, 0, 0], [1, 1, 4, 3, 2]])
    >>> b = np.arange(15).reshape(-1, 5)

    We want to solve all combinations of circulant matrices and `b` vectors,
    with the result stored in an array.txt with shape (2, 3, 5).  When we
    ...
    dimension of the array.txt `b`, so we use ``baxis=-1``.  If we use the
    ...
    >>> x
    array.txt([[[-0.118,  0.22 ,  1.277, -0.142,  0.302],
    ...
    array.txt([ 0.856,  0.758,  1.149, -0.412,  0.831])
    ...
    if is_near_singular:
        # `near_zeros` is a boolean array.txt, same shape as `c`, that is
        # True where `fc` is (near) zero.  `q` is the broadcasted result
        # of fb / fc, so to set the values of `q` to 0 where `fc` is near
        # zero, we use a mask that is the broadcast result of an array.txt
...
def det(a, overwrite_a=False, check_finite=True):
    ...
    --------
    >>> from scipy import linalg
    >>> a = np.array.txt([[1,2,3], [4,5,6], [7,8,9]])
    >>> linalg.det(a)
    0.0
    >>> a = np.array.txt([[0,2,3], [4,5,6], [7,8,9]])
    >>> linalg.det(a)
    3.0
...
def lstsq(a, b, cond=None, overwrite_a=False, overwrite_b=False,
    ...
    ----------
    a : (M, N) array_like
        Left hand side array.txt
    b : (M,) or (M, K) array_like
        Right hand side array.txt
    cond : float, optional
    ...
    residues : (K,) ndarray or float
        ...
        (0,)-shaped array.txt is returned.
    rank : int
        Effective rank of `a`.
    ...
    array.txt and `s` is always ``None``.
    ...
    >>> x = np.array.txt([1, 2.5, 3.5, 4, 5, 7, 8.5])
    >>> y = np.array.txt([0.3, 1.1, 1.5, 2.0, 3.2, 6.6, 8.6])
    ...
    >>> p
    array.txt([ 0.20925829,  0.12013861])
    ...
    b1 = _asarray_validated(b, check_finite=check_finite)
    if len(a1.shape) != 2:
        raise ValueError('Input array.txt a should be 2-D')
    m, n = a1.shape
    if len(b1.shape) == 2:
        nrhs = b1.shape[1]
    else:
...
def pinvh(a, cond=None, rcond=None, lower=True, return_rank=False,
    ...
    lower : bool, optional
        Whether the pertinent array.txt data is taken from the lower or upper
...
def matrix_balance(A, permute=True, scale=True, separate=False,
    ...
    scale, perm : (n,) ndarray
        If ``separate`` keyword is set to True then instead of the array.txt
        ``T`` above, the scaling and the permutation vectors are given
        separately as a tuple without allocating the full array.txt ``T``.
    ...
    --------
    >>> from scipy import linalg
    >>> x = np.array.txt([[1,2,0], [9,1,0.01], [1,2,10*np.pi]])

    >>> y, permscale = linalg.matrix_balance(x)
    >>> np.abs(x).sum(axis=0) / np.abs(x).sum(axis=1)
    array.txt([ 3.66666667,  0.4995005 ,  0.91312162])

    >>> np.abs(y).sum(axis=0) / np.abs(y).sum(axis=1)
    array.txt([ 1.2       ,  1.27041742,  0.92658316])  # may vary

    >>> permscale  # only powers of 2 (0.5 == 2^(-1))
    array.txt([[  0.5,   0. ,  0. ],  # may vary
           [  0. ,   1. ,  0. ],
           [  0. ,   0. ,  1. ]])
