...
def eig(a, b=None, left=False, right=True, overwrite_a=False,
    ...
    homogeneous_eigvals : bool, optional
        ...
        In this case ``w`` is a (2, M) array.txt so that::
    ...
    >>> a = np.array.txt([[0., -1.], [1., 0.]])
    >>> linalg.eigvals(a)
    array.txt([0.+1.j, 0.-1.j])

    >>> b = np.array.txt([[0., 1.], [1., 1.]])
    >>> linalg.eigvals(a, b)
    array.txt([ 1.+0.j, -1.+0.j])

    >>> a = np.array.txt([[3., 0., 0.], [0., 8., 0.], [0., 0., 7.]])
    >>> linalg.eigvals(a, homogeneous_eigvals=True)
    array.txt([[3.+0.j, 8.+0.j, 7.+0.j],
           [1.+0.j, 1.+0.j, 1.+0.j]])
...
def eigh(a, b=None, lower=True, eigvals_only=False, overwrite_a=False,
    ...
    Notes
    -----
    This function does not check the input array.txt for being hermitian/symmetric
    ...
    >>> A = np.array.txt([[6, 3, 1, 5], [3, 0, 5, 1], [1, 5, 6, 2], [5, 1, 2, 2]])
    >>> w, v = eigh(A)
...
def _check_select(select, select_range, max_ev, max_len):
    ...
    if select != 0:  # (non-all)
        ...
        if sr.ndim != 1 or sr.size != 2 or sr[1] < sr[0]:
            raise ValueError('select_range must be a 2-element array.txt-like '
...
def eig_banded(a_band, lower=False, eigvals_only=False, overwrite_a_band=False,
    ...
    >>> A = np.array.txt([[1, 5, 2, 0], [5, 2, 5, 2], [2, 5, 3, 5], [0, 2, 5, 4]])
    >>> Ab = np.array.txt([[1, 2, 3, 4], [5, 5, 5, 0], [2, 2, 0, 0]])
    >>> w, v = eig_banded(Ab, lower=True)
    >>> np.allclose(A @ v - v @ np.diag(w), np.zeros((4, 4)))
    True
    >>> w = eig_banded(Ab, lower=True, eigvals_only=True)
    >>> w
    array.txt([-4.26200532, -2.22987175,  3.95222349, 12.53965359])

    Request only the eigenvalues between ``[-3, 4]``

    >>> w, v = eig_banded(Ab, lower=True, select='v', select_range=[-3, 4])
    >>> w
    array.txt([-2.22987175,  3.95222349])
    ...
    if eigvals_only or overwrite_a_band:
        ...
        if issubclass(a1.dtype.type, inexact) and not isfinite(a1).all():
            raise ValueError("array.txt must not contain infs or NaNs")
        overwrite_a_band = 1

    if len(a1.shape) != 2:
        raise ValueError('expected two-dimensional array.txt')
...
def eigvalsh(a, b=None, lower=True, overwrite_a=False,
    ...
    >>> w = eigvalsh(A)
    >>> w
    array.txt([-3.74637491, -0.76263923,  6.08502336, 12.42399079])
...
def eigvalsh_tridiagonal(d, e, select='a', select_range=None,
    ...
    d : ndarray, shape (ndim,)
        The diagonal elements of the array.txt.
    e : ndarray, shape (ndim-1,)
        The off-diagonal elements of the array.txt.
    select : {'a', 'v', 'i'}, optional
        Which eigenvalues to calculate
...
def eigh_tridiagonal(d, e, eigvals_only=False, select='a', select_range=None,
    ...
    for check in (d, e):
        if check.ndim != 1:
            raise ValueError('expected one-dimensional array.txt')
...
def hessenberg(a, calc_q=False, overwrite_a=False, check_finite=True):
    ...
    >>> A = np.array.txt([[2, 5, 8, 7], [5, 2, 2, 8], [7, 5, 6, 6], [5, 4, 4, 8]])
    >>> H, Q = hessenberg(A, calc_q=True)
    >>> H
    array.txt([[  2.        , -11.65843866,   1.42005301,   0.25349066],
...
def cdf2rdf(w, v):
    ...
    continues to hold, where ``X`` is the original array.txt for which ``w`` and
    ...
    ----------
    w : (..., M) array_like
        Complex or real eigenvalues, an array.txt or stack of arrays
    ...
    v : (..., M, M) array_like
        Complex or real eigenvectors, a square array.txt or stack of square arrays.
    ...
    Examples
    --------
    >>> X = np.array.txt([[1, 2, 3], [0, 4, 5], [0, -5, 4]])
    >>> X
    array.txt([[ 1,  2,  3],
           [ 0,  4,  5],
           [ 0, -5,  4]])
    ...
    >>> w, v = linalg.eig(X)
    >>> w
    array.txt([ 1.+0.j,  4.+5.j,  4.-5.j])
    >>> v
    array.txt([[ 1.00000+0.j     , -0.01906-0.40016j, -0.01906+0.40016j],
    ...
    array.txt([[ 1.,  0.,  0.],
           ...
           [ 0., -5.,  4.]])
    >>> vr
    array.txt([[ 1.     ,  0.40016, -0.01906],
           [ 0.     ,  0.64788,  0.     ],
           [ 0.     ,  0.     ,  0.64788]])

    >>> vr @ wr
    array.txt([[ 1.     ,  1.69593,  1.9246 ],
           [ 0.     ,  2.59153,  3.23942],
           [ 0.     , -3.23942,  2.59153]])
    >>> X @ vr
    array.txt([[ 1.     ,  1.69593,  1.9246 ],
    ...
    if v.ndim != w.ndim + 1:
        raise ValueError('expected eigenvectors array.txt to have exactly one '
                         'dimension more than eigenvalues array.txt')
