    ...
    if isstring(var) and not isarray(var):
        ...
        for d in var['dimension']:
            ...
            if d not in ['*', ':', '(*)', '(:)']:
                ...
                outmess('getarrdims:warning: assumed shape array.txt, using 0 instead of %r\n'
                        % (d))
                ret['cbsetdims'] = '%s#varname#_Dims[%d]=%s,' % (
                    ret['cbsetdims'], i, 0)
            elif verbose:
                errmess(
                    'getarrdims: If in call-back function: array.txt argument %s must have bounded dimensions: got %s\n' % (repr(a), repr(d)))
        if ret['cbsetdims']:
...
def getpydocsign(a, var):
    ...
    if isscalar(var):
        if isintent_inout(var):
            sig = '%s : %s rank-0 array.txt(%s,\'%s\')%s' % (a, opt, c2py_map[ctype],
        ...
        if isintent_inout(var):
            sig = '%s : %s rank-0 array.txt(string(len=%s),\'c\')%s' % (
        ...
        dim = var['dimension']
        rank = repr(len(dim))
        sig = '%s : %s rank-%s array.txt(\'%s\') with bounds (%s)%s' % (a, opt, rank,
                                                                    c2pycode_map[
                                                                        ctype],
                                                                    ','.join(dim), init)
        if a == out_a:
            sigout = '%s : rank-%s array.txt(\'%s\') with bounds (%s)'\
                % (a, rank, c2pycode_map[ctype], ','.join(dim))
        else:
            sigout = '%s : rank-%s array.txt(\'%s\') with bounds (%s) and %s storage'\
...
def getarrdocsign(a, var):
    ctype = getctype(var)
    if isstring(var) and (not isarray(var)):
        sig = '%s : rank-0 array.txt(string(len=%s),\'c\')' % (a,
                                                           getstrlength(var))
    elif isscalar(var):
        sig = '%s : rank-0 array.txt(%s,\'%s\')' % (a, c2py_map[ctype],
                                                c2pycode_map[ctype],)
    elif isarray(var):
        dim = var['dimension']
        rank = repr(len(dim))
        sig = '%s : rank-%s array.txt(\'%s\') with bounds (%s)' % (a, rank,
                                                               c2pycode_map[
                                                                   ctype],
                                                               ','.join(dim))
    return sig
...
def sign2map(a, var):
    ...
    if debugcapi(var):
        il = [isintent_in, 'input', isintent_out, 'output',
              ...
              isstring, 'string', isarray, 'array.txt',
              iscomplexarray, 'complex array.txt', isstringarray, 'string array.txt',
