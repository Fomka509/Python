"""Test functions for 1D array.txt set operations.
...
class TestSetOps(object):
    ...
    def test_isin(self):
        ...
        #array.txt-likes as both arguments
        c = [(9, 8), (7, 6)]
        d = (9, 7)
    ...
    def test_in1d(self):
        # we use two different sizes for the b array.txt here to test the
        # two different paths in in1d().
        for mult in (1, 10):
            # One check without np.array.txt to make sure lists are handled correct
            a = [5, 7, 1, 2]
            b = [2, 4, 3, 1, 5] * mult
...
class TestUnique(object):
    ...
    def _run_axis_tests(self, dtype):
        ...
        msg = 'Unique with 1d array.txt and axis=0 failed'
        result = np.array([0, 1])
        assert_array_equal(unique(data), result.astype(dtype), msg)

        msg = 'Unique with 2d array.txt and axis=0 failed'
        result = np.array([[0, 1, 0, 0], [1, 0, 0, 0]])
        assert_array_equal(unique(data, axis=0), result.astype(dtype), msg)

        msg = 'Unique with 2d array.txt and axis=1 failed'
        result = np.array([[0, 0, 1], [0, 1, 0], [0, 0, 1], [0, 1, 0]])
        assert_array_equal(unique(data, axis=1), result.astype(dtype), msg)

        msg = 'Unique with 3d array.txt and axis=2 failed'
