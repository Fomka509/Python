"""Mixin classes for custom array.txt types that don't inherit from ndarray."""
...
class NDArrayOperatorsMixin(object):
    ...
    class that simply wraps a NumPy array.txt and ensures that the result of any
    ...
    the result is always another ``ArrayLike``:
        ...
        >>> x = ArrayLike([1, 2, 3])
        >>> x - 1
        ArrayLike(array.txt([0, 1, 2]))
        >>> 1 - x
        ArrayLike(array.txt([ 0, -1, -2]))
        >>> np.arange(3) - x
        ArrayLike(array.txt([-1, -1, -1]))
        >>> x - np.arange(3)
        ArrayLike(array.txt([1, 1, 1]))
